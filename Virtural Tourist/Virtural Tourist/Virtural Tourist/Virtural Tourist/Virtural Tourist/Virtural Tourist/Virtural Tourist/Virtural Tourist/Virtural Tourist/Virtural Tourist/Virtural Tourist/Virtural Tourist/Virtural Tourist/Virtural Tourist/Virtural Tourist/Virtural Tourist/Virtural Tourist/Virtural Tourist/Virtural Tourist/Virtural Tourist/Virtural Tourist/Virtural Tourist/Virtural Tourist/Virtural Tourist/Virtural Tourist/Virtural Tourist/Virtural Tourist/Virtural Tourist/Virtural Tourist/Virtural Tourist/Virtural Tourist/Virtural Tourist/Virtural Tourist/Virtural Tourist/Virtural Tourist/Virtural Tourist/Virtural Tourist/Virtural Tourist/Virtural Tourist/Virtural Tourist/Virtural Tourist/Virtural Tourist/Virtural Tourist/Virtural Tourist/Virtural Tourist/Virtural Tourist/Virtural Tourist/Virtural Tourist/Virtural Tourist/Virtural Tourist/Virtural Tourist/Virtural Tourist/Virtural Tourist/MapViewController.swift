...
class MapViewController: BaseViewController, MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, didSelect view: MKAnnotationView) {
        ...
        fr.predicate = NSPredicate(format: "lat >= \(lat - precision) AND lat <= \(lat + precision) AND long >= \(long - precision) AND long <= \(long + precision)")
        
        do {
            let fer = try stack.context.fetch(fr) as NSArray!
